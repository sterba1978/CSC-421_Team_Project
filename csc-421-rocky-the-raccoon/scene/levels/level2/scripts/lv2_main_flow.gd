...
const JOURNAL_CASE_TITLE := "Patty's PictoSnap Case"
const JOURNAL_CASE_SUMMARY := "Patty lost access to her PictoSnap account after suspicious messages asked for login details. Use the journal to compare who is real and who is impersonating someone."
const JOURNAL_QUESTIONS := [
	"Which message led to Patty's account being hacked?",
	"What details show the sender is impersonating someone?",
	"What should Patty do before sharing account information online?"
]
const JOURNAL_PLACEHOLDERS := [
	"Example: Message 1 / Rodney",
	"Example: fake profile, odd wording, AI image details, asks for login...",
	"Example: verify with the friend, never share login info, contact support..."
]
...
@export_file("*.tscn") var level_complete_cinematic_scene_path: String = "res://scene/levels/level2/lv2_complete_cinematic_scene.tscn"
...
func _ready() -> void:
	...
	if not Autoload.level2_ended.is_connected(_on_level2_ended):
		Autoload.level2_ended.connect(_on_level2_ended)
...
func _grade_journal_answers() -> void:
	if _journal_ui != null and _journal_ui.has_method("get_question_answers"):
		Autoload.grade_case_journal(2, _journal_ui.get_question_answers())
