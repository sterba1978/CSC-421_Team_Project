...
const FALLBACK_LEVEL_3_SCENE := preload("res://scene/levels/level3/mainlv3.tscn")
...
@export_file("*.tscn") var next_level_scene_path: String = "res://scene/levels/level3/mainlv3.tscn"
@export_file("*.ogv") var next_client_video_path: String = "res://assets/video/peter.Ogv"
...
func _get_next_client_video_stream() -> VideoStream:
	...
	if not ResourceLoader.exists(next_client_video_path):
		push_warning("lv2_complete_cinematic_scene.gd: Next client video '%s' was not found." % next_client_video_path)
		return null

	_client_video_stream = load(next_client_video_path) as VideoStream
	if _client_video_stream == null:
		push_warning("lv2_complete_cinematic_scene.gd: Next client video '%s' is not a valid VideoStream resource." % next_client_video_path)
		return null
...
func _load_next_level() -> void:
	...
	if error != OK:
		error = get_tree().change_scene_to_packed(FALLBACK_LEVEL_3_SCENE)

	if error != OK:
		SceneTransitionState.reset()
		push_warning("lv2_complete_cinematic_scene.gd: Failed to load next level scene '%s' (error %d)." % [next_level_scene_path, error])
		if fade_out_duration > 0.0:
