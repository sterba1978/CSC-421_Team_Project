...
@onready var bear = get_node_or_null("../Peter")
...
func _ready() -> void:
	...
	if bear != null and bear.has_signal("bear_talk"):
		bear.bear_talk.connect(_on_bear_talk)
	else:
		push_warning("Level 3 DialogueManager: Peter node or bear_talk signal was not found.")
